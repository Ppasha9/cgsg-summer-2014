...
BOOL RK2_AnimInit( HWND hWnd )
{
  ...
  RK2_Anim.ShaderDef = RK2_ShadProgInit("shaders\\default.vert", "shaders\\default.frag");
...
VOID RK2_RndSendGlobInfo(UINT ShaderDest, rk2ANIM *Ani)
{
  ...
  if (ShaderDest)
  {
    ...
    loc = glGetUniformLocation(ShaderDest, "Time");
    if (loc != -1)
    ...
    loc = glGetUniformLocation(ShaderDest, "WaterLevel");
    if (loc != -1)
      glUniform1f(loc, Ani->WaterLevel);

    loc = glGetUniformLocation(ShaderDest, "MatrWVP");
    if (loc != -1)
      glUniformMatrix4fv(loc, 1, FALSE, Ani->RndMatrRes.A[0]);

    loc = glGetUniformLocation(ShaderDest, "MatrWorldInverseTranspose");
    if (loc != -1)
      glUniformMatrix4fv(loc, 1, FALSE, MatrTranspose(MatrInverse(Ani->RndMatrWorld)).A[0]);

    loc = glGetUniformLocation(ShaderDest, "MatrWorld");
    if (loc != -1)
      glUniformMatrix4fv(loc, 1, FALSE, Ani->RndMatrWorld.A[0]);

    loc = glGetUniformLocation(ShaderDest, "MatrView");
    if (loc != -1)
      glUniformMatrix4fv(loc, 1, FALSE, Ani->RndMatrView.A[0]);
