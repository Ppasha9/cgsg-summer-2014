...
*/
static VOID LoadMaterials( rk2GOBJ *G, CHAR *FileName )
{
  FILE *F;
  rk2GMATERIAL DefMat, Mat;
...
BOOL RK2_GObjLoad( rk2GOBJ *G, CHAR *FileName )
{
  ...
  size = 
    sizeof(rk2VEC) * vn +                        /*   вершины               vn */
    sizeof(rk2VEC) * vnn +                       /*   нормали               vnn */
    sizeof(rk2UV) * vtn +                     /*   текстурные координаты vtn */
  ...
  RK2_DefaultColor = ColorSet(1, 1, 1);
  for (p = 0; p < pn; p++)
  {
    INT minv, maxv, j;
    rk2GPRIM prim;
    ...
    RK2_GPrimCreate(&prim, RK2_PRIM_TRIMESH, vn, fn * 3);
    ...
    RK2_GObjAddPrim(G, &prim);
  }
  /* освобождаем память из-под прочитанных данных */
  free(ReadV);
  return TRUE;
} /* End of 'RK2_GeomLoad' function */
